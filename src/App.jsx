        ...
        {[...items, ...items].map((it, idx) => (
          <a key={`${it.org}-${idx}`} href={it.link || '#'} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-3 pr-2" title={`${it.role} — ${it.org}`}>
            ...
            <div className="leading-tight">
              <div className="text-sm font-semibold text-emerald-950 dark:text-emerald-50">{it.org}</div>
              <div className="text-[11px] text-emerald-900/80 dark:text-emerald-100/80">{it.role}</div>
            </div>
          </a>
        ))}
      </div>
...
function ContactPage() {
  ...
  return (
    <section className="max-w-3xl mx-auto px-6 md:px-8 py-14">
      <h1 className="text-3xl font-semibold text-emerald-950 dark:text-emerald-50">Contact</h1>
      <p className="mt-2 text-emerald-900 dark:text-emerald-100/90">
      ...
      <form action={action} method="POST" className="mt-8 grid gap-4">
        <input name="name" required placeholder="Your name" className="rounded-xl px-4 py-3 ring-1 ring-black/10 bg-white dark:bg-emerald-900 dark:text-emerald-50"/>
        <input name="email" type="email" required placeholder="you@example.com" className="rounded-xl px-4 py-3 ring-1 ring-black/10 bg-white dark:bg-emerald-900 dark:text-emerald-50"/>
        <textarea name="message" required placeholder="How can I help?" className="rounded-xl px-4 py-3 h-36 ring-1 ring-black/10 bg-white dark:bg-emerald-900 dark:text-emerald-50"/>
        <button className="bg-emerald-700 text-white rounded-xl px-5 py-3 w-fit hover:opacity-90">Send</button>
      </form>

      <div className="mt-8 text-sm text-emerald-900/80 dark:text-emerald-100/80">
...
function PublicationsPage() {
  ...
  return (
    <section className="max-w-6xl mx-auto px-6 md:px-8 py-14">
      <h1 className="text-3xl font-semibold text-emerald-950 dark:text-emerald-50">Writing Published In</h1>
      <p className="mt-2 text-emerald-900 dark:text-emerald-100/90">A selection of outlets featuring my work.</p>

      <Card className="mt-6 p-4">
        <div className="flex flex-wrap items-center gap-3">
          <a href={LINKS.scholar} target="_blank" rel="noopener noreferrer" className="group inline-flex items-center gap-2 rounded-xl px-3 py-2 ring-1 ring-black/10 dark:ring-white/10 bg-white dark:bg-emerald-800 text-emerald-900 dark:text-emerald-50 hover:shadow-sm">
          ...
          <a href={LINKS.orcid} target="_blank" rel="noopener noreferrer" className="group inline-flex items-center gap-2 rounded-xl px-3 py-2 ring-1 ring-black/10 dark:ring-white/10 bg-white dark:bg-emerald-800 text-emerald-900 dark:text-emerald-50 hover:shadow-sm">
          ...
          <a href={LINKS.linkedin} target="_blank" rel="noopener noreferrer" className="group inline-flex items-center gap-2 rounded-xl px-3 py-2 ring-1 ring-black/10 dark:ring-white/10 bg-white dark:bg-emerald-800 text-emerald-900 dark:text-emerald-50 hover:shadow-sm">
      ...
      <div className="mt-6 text-[11px] text-emerald-900/80 dark:text-emerald-100/75">
  ...
  return (
    <section id="journey" className="max-w-6xl mx-auto px-6 md:px-8 py-14">
      <div className="grid md:grid-cols-2 gap-10 items-center">
        <div>
          <h2 className="text-3xl md:text-4xl font-semibold text-emerald-950 dark:text-emerald-50">My Journey</h2>
          <p className="mt-3 text-emerald-900 dark:text-emerald-100/90">
        ...
        <div className="relative">
          ...
          <div className="mt-2 text-xs text-emerald-900/80 dark:text-emerald-100/75">Moments with teams and projects that shaped my work.</div>
        </div>
      </div>
    </section>
  )
...
export default function App() {
  ...
  return (
    <div className={`${brand.bg} min-h-screen`}>
      <header className="sticky top-0 z-40 backdrop-blur bg-white/80 dark:bg-emerald-900/80 border-b border-black/5 dark:border-white/10">
        <div className="max-w-6xl mx-auto flex items-center justify-between px-6 md:px-8 h-16">
          <a href="#/" className="group inline-flex items-center gap-2">
            ...
            <div className="font-medium tracking-tight text-emerald-900 dark:text-emerald-50 group-hover:opacity-90 transition">Imeth Illamperuma</div>
          </a>

          <nav className="hidden md:flex items-center gap-6 text-emerald-900 dark:text-emerald-100">
            ...
            <button
              onClick={() => setDark(v => !v)}
              className="ml-2 rounded-lg px-3 py-1 ring-1 ring-black/10 dark:ring-white/10 bg-white dark:bg-emerald-800 text-emerald-900 dark:text-emerald-50"
              title="Toggle theme"
            >
              {dark ? 'Light' : 'Dark'}
            </button>
        ...
        <section id="home" className="relative overflow-hidden">
          <div className="max-w-6xl mx-auto px-6 md:px-8 py-16 md:py-20 flex flex-col gap-10">
            ...
            <div className="flex flex-col md:flex-row items-center gap-8">
              ...
              <div>
                <motion.h1 initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="text-4xl md:text-5xl font-semibold tracking-tight text-emerald-950 dark:text-emerald-50">
                  Hey, I’m Imeth! I connect science, humanity, and innovation to shape the future of medicine.
                </motion.h1>
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.1 }} className="mt-2 text-lg md:text-xl text-emerald-900 dark:text-emerald-100/90">
                ...
                <motion.p initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2 }} className="mt-4 max-w-3xl text-emerald-900 dark:text-emerald-100/90">
                ...
                <div className="mt-8 flex flex-wrap items-center gap-3">
                  ...
                  <a href={`${BASE}Imeth-Illamperuma-CV.pdf`} className="inline-flex items-center gap-2 rounded-xl px-4 py-2 ring-1 ring-black/10 hover:ring-black/20 bg-white dark:bg-emerald-800 dark:text-emerald-50"><FileDown className="w-4 h-4" /> Download CV</a>
                </div>
              </div>
            </div>
          ...
          <section id="offerings" className="max-w-6xl mx-auto px-6 md:px-8 py-14">
            ...
            <div className="grid md:grid-cols-3 gap-6">
              <Card><h3 className="font-semibold text-emerald-950 dark:text-emerald-50">Research Collaboration</h3><p className="mt-2 text-emerald-900 dark:text-emerald-100/90">Imaging + multi-omics for early detection along the brain–gut axis.</p></Card>
              <Card><h3 className="font-semibold text-emerald-950 dark:text-emerald-50">Policy & Advocacy</h3><p className="mt-2 text-emerald-900 dark:text-emerald-100/90">Translating findings into practical guidance for equitable systems.</p></Card>
              <Card><h3 className="font-semibold text-emerald-950 dark:text-emerald-50">Talks & Workshops</h3><p className="mt-2 text-emerald-900 dark:text-emerald-100/90">Prevention-first medicine, harm reduction on campus, digital health equity.</p></Card>
            </div>
          </section>
        </section>
      )}
      ...
      <footer className="border-t border-black/5 dark:border-white/10">
        <div className="max-w-6xl mx-auto px-6 md:px-8 py-10 text-sm text-emerald-900/80 dark:text-emerald-100/80 flex flex-col md:flex-row items-center justify-between gap-3">
